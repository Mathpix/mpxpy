...
class ScsFile:
    ...
    def cropped_image(self, page: int, top_left_x: int, top_left_y: int,
                      width: int = 0, height: int = 0) -> bytes:
        """Get a cropped region from a specific page.

        Args:
            page: Page number (0-indexed)
            top_left_x: X coordinate of crop top-left corner
            top_left_y: Y coordinate of crop top-left corner
            width: Crop width (0 = original_width - top_left_x)
            height: Crop height (0 = original_height - top_left_y)

        Returns:
            bytes: The cropped JPEG image bytes
        """
        logger.debug(f"Getting cropped image for file {self.file_id} page {page}")
        endpoint = urljoin(
            self.auth.files_api_url,
            f'/files/v1/cropped/{self.file_id}-{page}.jpg'
        )
        params = {
        ...
        if response.status_code == 404:
            raise ConversionIncompleteError("File or page not found")
        if response.status_code == 400:
            raise ValidationError("Invalid crop dimensions")
        return response.content
